...
SfmlApp::SfmlApp(
	...
	: window_({ window_size.first, window_size.second }, "Conway's Game of Life")
	, cell_size_(cell_size)
	, game(window_size.first / cell_size.first, window_size.second / cell_size.second)
{
}
...
void SfmlApp::init(sf::Color living_cell_color, sf::Color dead_cell_color, sf::Color text_color, unsigned character_size)
{
	this->living_cell_color_ = living_cell_color;
	this->dead_cell_color_ = dead_cell_color;
	gui_text_.setColor(text_color);
	if (font_.loadFromFile("../../data/OpenSans-Regular.ttf"))
	{
		...
		gui_text_.move(10, 2);
		gui_text_.setCharacterSize(character_size);
...
void SfmlApp::run(int duration_between_update)
{
	unsigned time_elapsed_since_update = 0;
	bool simulation_active(false);
	// TODO: it would be nice to make this configurable in the future.
	unsigned duration_in_millis_between_updates = duration_between_update;
		...
		while (window_.pollEvent(event))
		{
			...
			if (event.type == sf::Event::MouseButtonPressed)
			{
				if (event.mouseButton.button == sf::Mouse::Button::Left)
				{
					...
					if (game.lastMatrix[clicked_cell_x][clicked_cell_y]) {
						setCellColor(clicked_cell_x, clicked_cell_y, dead_cell_color_);
						game.lastMatrix[clicked_cell_x][clicked_cell_y] = 0;
					}
					else {
						setCellColor(clicked_cell_x, clicked_cell_y, living_cell_color_);
						game.lastMatrix[clicked_cell_x][clicked_cell_y] = 1;
					}
				}
			}
		}
...
void SfmlApp::updateWorld()
{
	...
	for (int i = 0; i < game.matrixH; i++) {
		for (int j = 0; j < game.matrixW; j++) {
			if(game.matrix[i][j])
